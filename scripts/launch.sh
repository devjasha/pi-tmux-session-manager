...
# Always create a unique session so prefix + y spawns fresh sessions.
# Re-attaching to old sessions is the picker's job (prefix + u).
session="${prefix}$(session_hash "$path")-$(date +%s)$RANDOM"
